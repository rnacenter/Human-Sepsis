...
if(!is.null(celllist)){
	celllistUse = unique(read.table(celllist,sep="\t",header=F)[,1])
...
print("RunHarmony")
seuset = RunHarmony(seuset,"orig.ident",assay.use=assay,theta=theta,lambda=lambda)

print("RunUMAP")
seuset <- RunUMAP(seuset, dims = pcadim, n.neighbors = nneighbors, reduction.name = "umap3d", n.components = 3, reduction.key = "umap3d_",reduction="harmony")
seuset <- RunUMAP(seuset, dims = pcadim, n.neighbors = nneighbors,reduction="harmony")

print("RunTSNE")
seuset <- RunTSNE(object = seuset,dims = pcadim,reduction.name = "tsne3d",dim.embed=3, reduction.key = "tSNE3d_",check_duplicates = FALSE,reduction="harmony")
seuset <- RunTSNE(object = seuset,dims = pcadim,check_duplicates = FALSE,reduction="harmony")
...
write.table(data.frame(CellName=rownames(seuset@reductions$harmony@cell.embeddings),seuset@reductions$harmony@cell.embeddings),file="harmony.txt",sep="\t",quote=F,row.names=F)

dir.create("GraphClust")
seuset <- FindNeighbors(seuset, reduction = "harmony", dims = pcadim)
...
saveRDS(seuset, file = paste0("GraphClust.Harmony.SeuratObject.rds"))
